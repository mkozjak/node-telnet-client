...
function parseData(chunk, telnetObj) {
  ...
  if (telnetObj.telnetState === 'getprompt') {
    ...
    var promptIndex = stringData.search(telnetObj.prompt);

    if (promptIndex !== -1) {
      telnetObj.prompt = stringData.substring(promptIndex);
      telnetObj.telnetState = 'sendcmd';
      telnetObj.stringData = '';
      telnetObj.emit('ready', telnetObj.prompt);
    }
    else if (stringData.search(/login[: ]*$/i) !== -1) {
